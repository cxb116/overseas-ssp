...
func (this_ *AsyncSink) run() {
	go func() {
		...
		for !this_.Closed() {
			...
			// 先保存长度，用于后续 shrink 判断
			buffLen := len(this_.swapBuff[1])
			...
			// 清空 buffer
			for i := range this_.swapBuff[1] {
				this_.swapBuff[1][i] = nil
			}
			this_.swapBuff[1] = this_.swapBuff[1][:0]

			// 当容量大于 2560 且实际使用量小于一半时，进行 shrink
			capSize := cap(this_.swapBuff[1])
			if capSize > 2560 && buffLen < capSize/2 {
